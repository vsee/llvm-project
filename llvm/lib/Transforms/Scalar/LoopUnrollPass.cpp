...
#define AUTO2_LOG(msg) \
    if (auto2_config) { \
        auto2Log.open(auto2_config->logfile, std::ios::out | std::ios::app); \
        auto2Log << msg; \
        auto2Log.close(); \
    }
...
std::once_flag auto2_init;
Optional<A2Config> auto2_config;
std::ofstream auto2Log;

void print_header() {
    std::time_t now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    AUTO2_LOG("\nAuto2 log: LoopUnrollPass.cpp:analyzeLoopUnrollCost # "
              << std::ctime(&now) << "\n");
}

void load_config() {
  std::ifstream in (AUTO2_CONFIG_FILE_NAME, std::ios::in);
  if (in.is_open()) {
    std::string logfile = "NA";
    std::string pname = "NA";
    std::string data = "";
    ...
    while(std::getline(in, line)) {
      ...
      if (count == 0) logfile = line;
      else if (count == 1) pname = line;
      else  data += line + "\n";
    ...
    auto2_config = {{logfile, pname, data}};   
  } else {
      DEBUG_WITH_TYPE("auto2", dbgs() << "ERROR: Loading auto2 config failed.\n");
      auto2_config = None;
  }
...
void initialise_auto2() {
  DEBUG_WITH_TYPE("auto2", dbgs() << "Loading aut2 config file " << AUTO2_CONFIG_FILE_NAME << "\n");
  load_config();
  if (auto2_config) { // loading successful
    print_header();
  }
}
#else
...
static Optional<EstimatedUnrollCost> analyzeLoopUnrollCost_original(
  ...
  if (!L->isInnermost()) {
    AUTO2_LOG("Early exit: Innermost loop\n");
    return None;
  }

  // Don't simulate loops with a big or unknown tripcount
  if (!TripCount || TripCount > MaxIterationsCountToAnalyze) {
    AUTO2_LOG("Early exit: tripcount unknown or too big\n");
    return None;
  }
...
static Optional<EstimatedUnrollCost> analyzeLoopUnrollCost(
  ...
  AUTO2_LOG("analyzeLoopUnrollCost called\n");
    ...
    if (!auto2_config) {
      DEBUG_WITH_TYPE("auto2", dbgs() << "WARNING: Auto2 config not found. Using original values.\n");     
      Cost = Cost_orig;
    } else {
      ...
      if (!Cost_orig) {
        ...
        if (auto2_config->prior_name == "Original") {
          ...
          std::istringstream f(auto2_config->value_data);
          std::string line;
          int counter = 0;
          while (std::getline(f, line)) {
              ...
              if (counter == 0) {
                UnrolledCost = static_cast<unsigned>(std::atoi(line.c_str()));
              } else if (counter == 1) {
                RolledDynamicCost = static_cast<unsigned>(std::atoi(line.c_str()));
              } else {
                DEBUG_WITH_TYPE("auto2", dbgs() << "ERROR: invalid value configuration in auto2 config.\n");
              }
              counter++;
          }
    ...
    return analyzeLoopUnrollCost_original(
      L, TripCount, DT, SE, EphValues,
      TTI, MaxUnrolledLoopSize, 
      MaxIterationsCountToAnalyze);
#endif
...
bool llvm::computeUnrollCount(
  ...
  if (FullUnrollTripCount && FullUnrollTripCount <= UP.FullUnrollMaxCount) {
...
#ifdef AUTO2
    std::call_once(auto2_init, initialise_auto2);
#endif

    AUTO2_LOG("full unrolling considered\n")
    AUTO2_LOG("Loop Name: " << L->getName().str() << "\n");
    AUTO2_LOG("Loop ID: " << L->getLoopID() << "\n");
    AUTO2_LOG("Debug Start Loc: " << L->getStartLoc().getLine() << "," << L->getStartLoc().getCol() << "\n");
    AUTO2_LOG("unrolledLoopSize: " << unrolledLoopSize << "\n");
    ...
    if (unrolledLoopSize < UP.Threshold) {
      ...
      AUTO2_LOG("Loop fully unrolled without analysis\n");
